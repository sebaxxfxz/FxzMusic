...
class EqualizerViewModel : ViewModel() {
    ...
    fun toggleEnabled() {
        isEnabled = !isEnabled
        prefs?.edit()?.putBoolean(EXTRA_EQ_ENABLED, isEnabled)?.apply()
        try { hardwareEq?.enabled = isEnabled } catch (_: Exception) {}
        notifyService()
    }

    fun selectProfile(profile: EqProfile) {
        currentProfile = profile
        prefs?.edit()?.putString(EXTRA_EQ_PROFILE, profile.id)?.apply()
        applyCurrentProfileToHardware()
        notifyService()
    }

    fun updateBand(bandIndex: Int, gainDb: Float) {
        val newBands = currentProfile.bands.toMutableList()
        ...
        if (updatedProfile.isCustom && customProfiles.any { it.id == updatedProfile.id }) {
            ...
            saveProfilesToPrefs()
            prefs?.edit()?.putString(EXTRA_EQ_PROFILE, updatedProfile.id)?.apply()
        } else {
    ...
    fun saveCustomProfile(name: String) {
        ...
        saveProfilesToPrefs()
        prefs?.edit()?.putString(EXTRA_EQ_PROFILE, newProfile.id)?.apply()
        notifyService()
    }
    ...
    private fun saveProfilesToPrefs() {
        prefs?.edit()?.putString("custom_profiles", gson.toJson(customProfiles))?.apply()
    }
    ...
    override fun onCleared() {
        super.onCleared()
        try { hardwareEq?.release() } catch (_: Exception) {}
    }
}
