...
object LegacyMigrationManager {
    ...
    private suspend fun migrateLibraryPrefs(context: Context, db: FxzDatabase, gson: Gson) {
        ...
        val playlists = if (playlistJson.isNullOrBlank()) {
            ...
            try {
                ...
                android.util.Log.w("LegacyMigrationManager", "Failed to migrate library prefs", e)
                emptyList()
            }
        }
    ...
    private suspend fun migrateStatsPrefs(context: Context, db: FxzDatabase, gson: Gson) {
        ...
        val stats = try {
            ...
            android.util.Log.w("LegacyMigrationManager", "Failed to migrate stats prefs", e)
            emptyList()
        }
        ...
        val existingMeta = db.songMetaDao().getAll().associateBy { it.songId }
        db.songMetaDao().upsertAll(
            stats.map {
                val existing = existingMeta[it.songId]
                SongMetaEntity(
                    songId = it.songId,
                    playCount = it.playCount,
